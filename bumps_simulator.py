...
class Crew:
    def __init__(self, name, start_pos, division):
        ...
        self.blade_colors = BLADE_COLORS.get(name, ['#FFFFFF'])
...
class BumpsSimulator(tk.Tk):
    ...
    def setup_widgets(self):
        ...
        tk.Label(control_frame, text="Chart:", bg=BG_COLOR, fg=TEXT_COLOR).pack(side="left", padx=(0,5))
        ...
        tk.Label(control_frame, text="Click Action:", bg=BG_COLOR, fg=TEXT_COLOR).pack(side="left", padx=(20,5))
        ...
        undo_button = tk.Button(control_frame, text="Undo", bg="#30363D", fg=TEXT_COLOR, command=self.undo_last_move, relief="flat", padx=10)
        undo_button.pack(side="right", padx=(5,0))
        reset_button = tk.Button(control_frame, text="Reset All", bg="#30363D", fg=TEXT_COLOR, command=self.reset_all, relief="flat", padx=10)
    ...
    def draw_chart(self):
        ...
        for day in range(4):
            ...
            self.canvas.create_text(x_center, 10, text=f"Day {day+1}", fill=TEXT_COLOR, font=FONT_DIV)

        for day in range(1, 4):
            x = LEFT_MARGIN + day * day_width
            self.canvas.create_line(x, 20, x, canvas_height, fill=DIVIDER_LINE_COLOR, dash=(2, 4))
        ...
        for i, crew in enumerate(self.current_crews):
            if crew.division != last_div:
                ...
                self.canvas.create_line(40, y_pos - ROW_HEIGHT / 2, canvas_width, y_pos - ROW_HEIGHT / 2, fill=LINE_COLOR, dash=(4, 2))
        ...
        for div_num, (start, end) in div_boundaries.items():
            ...
            self.canvas.create_text(20, center_y, text=f"Division {div_num}", fill=TEXT_COLOR, font=FONT_DIV, anchor="center", angle=90)
        ...
        for crew, start_pos in start_positions.items():
            ...
            self.canvas.create_text(50, y_start, text=f"{start_pos + 1}", fill=TEXT_COLOR, font=FONT_MAIN, anchor="e")
            self.draw_blade(62, y_start, crew.blade_colors)
            self.canvas.create_text(75, y_start, text=crew.name, fill=TEXT_COLOR, font=FONT_MAIN, anchor="w")
            
            finish_pos = finish_positions[crew]
            y_finish = (finish_pos + 0.5) * ROW_HEIGHT + 20
            self.canvas.create_text(canvas_width - RIGHT_MARGIN + 8, y_finish, text=crew.name, fill=TEXT_COLOR, font=FONT_MAIN, anchor="w")
            self.draw_blade(canvas_width - 55, y_finish, crew.blade_colors)
            self.canvas.create_text(canvas_width - 35, y_finish, text=f"{finish_pos + 1}", fill=TEXT_COLOR, font=FONT_MAIN, anchor="e")
            
            self.draw_crew_line(crew, day_width, LINE_COLOR, "line")

    def draw_crew_line(self, crew, day_width, color, tag):
        y_offset = 20
        ...
        self.canvas.create_line(line_points, fill=color, width=2.5 if tag == "highlight" else 1.5, tags=tag)

    def draw_blade(self, x, y, colors):
        blade_width, blade_height = 12, 8
        if not colors: colors = ['#FFFFFF']
        num_colors = len(colors)
        stripe_width = blade_width / num_colors
    ...
    def handle_hover(self, event):
        ...
        if interaction:
            ...
            day_width = self.get_day_width()
            self.draw_crew_line(chaser, day_width, HIGHLIGHT_COLOR, "highlight")
            self.draw_crew_line(chased, day_width, HIGHLIGHT_COLOR, "highlight")
